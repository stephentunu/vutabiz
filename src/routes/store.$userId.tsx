...
function StorePage() {
  ...
  useEffect(() => {
    supabase.from("profiles").select("full_name,town,county_id").eq("id", userId).maybeSingle().then(({ data }) => setProfile(data as Profile | null));
    supabase.from("listings").select("id,title,price,image_url,status").eq("seller_id", userId).eq("status", "active").order("created_at", { ascending: false }).then(({ data }) => setItems((data as Row[]) ?? []));
  }, [userId]);
  return (
      ...
      <main className="flex-1 bg-background py-10">
        <div className="mx-auto max-w-6xl px-4">
          <div className="rounded-2xl bg-gradient-to-br from-primary to-primary-dark text-white p-8 shadow-lg">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold mt-1">{profile?.full_name ?? "Seller"}</h1>
            {profile?.town && <div className="mt-2 flex items-center gap-2 text-white/80"><MapPin className="h-4 w-4" /> {profile.town}</div>}
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            ...
            {!items.length && <div className="col-span-full text-muted-foreground">No active listings yet.</div>}
          </div>
        </div>
      </main>
      <Footer />

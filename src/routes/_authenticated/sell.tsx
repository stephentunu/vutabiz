...
function SellPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("subcounties").select("id,county_id,name").order("name").then(
      ({ data }) => setSubCounties(data && data.length > 0 ? (data as SubCounty[]) : (STATIC_SUB_COUNTIES as SubCounty[])),
    ...
    (async () => {
      ...
      const acc: Ward[] = [];
      while (true) {
        const { data, error } = await supabase.from("wards").select("id,county_id,sub_county_id:subcounty_id,name").order("name").range(from, from + pageSize - 1);
        if (error || !data) break;
  ...
  const wardsForSubCounty = wards.filter(
    (w) => w.county_id === Number(countyId) && (subCountyId ? w.sub_county_id === Number(subCountyId) : true),
  );

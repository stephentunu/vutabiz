...
function SellPage() {
  ...
  const [town, setTown] = useState(""); const [imageUrl, setImageUrl] = useState("");
  ...
  useEffect(() => {
    supabase.from("categories").select("id,name,parent_id").order("name").then(({ data }) => setCats((data as Cat[]) ?? []));
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (data.user) supabase.from("profiles").select("county_id,ward_id,town").eq("id", data.user.id).maybeSingle().then(({ data: p }) => {
        if (p) { setCountyId(p.county_id ?? ""); setWardId(p.ward_id ?? ""); setTown(p.town ?? ""); }
      });
    });
  }, []);
  ...
  async function submitListing(e: React.FormEvent) {
    ...
    try {
      const res = await create({ data: {
        title, description: desc, price, category_id: categoryId ? Number(categoryId) : null,
        county_id: countyId ? Number(countyId) : null, ward_id: wardId ? Number(wardId) : null,
        town, image_url: imageUrl || null, distance_km: distance, risk, duration_days: days,
      } });
      setCreatedId(res.id); setFee(res.ad_fee_ksh);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 bg-background py-10">
        <div className="mx-auto max-w-3xl px-4">
          ...
          {!createdId ? (
            <form onSubmit={submitListing} className="mt-6 bg-card rounded-2xl shadow ring-1 ring-black/5 p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Sel label="County" v={countyId} on={(v) => { setCountyId(v); setWardId(""); }} opts={[{v:"",l:"Select"}, ...counties.map(c => ({v:c.id,l:c.name}))]} />
                <Sel label="Ward" v={wardId} on={setWardId} opts={[{v:"",l:"—"}, ...wardsForCounty.map(w => ({v:w.id,l:w.name}))]} />
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div className="pt-2 border-t border-border">
                <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Ad settings</div>
                <div className="grid grid-cols-3 gap-3">
                  <NumField label="Distance (km)" v={distance} on={setDistance} />
                  <Sel label="Risk" v={risk} on={(v) => setRisk(v as "low"|"medium"|"high")} opts={[{v:"low",l:"Low"},{v:"medium",l:"Medium"},{v:"high",l:"High"}]} />
              ...
              <button disabled={loading} className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-primary-dark hover:bg-primary text-white px-4 py-3 font-bold disabled:opacity-60">
            ...
            <div className="mt-6 bg-card rounded-2xl shadow ring-1 ring-black/5 p-6">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Pay <b>KSh {fee}</b> via M-Pesa Paybill <b>247247</b>, Account <b>{createdId.slice(0,8)}</b>, then paste the M-Pesa confirmation code below.</p>
              <div className="mt-4 flex gap-2">
                ...
                <button disabled={loading || !mpesa} onClick={payAd} className="rounded-xl bg-primary-dark text-white px-5 py-2.5 font-bold disabled:opacity-60">
                  Confirm payment
                </button>
              </div>
              <Link to="/dashboard" className="mt-4 inline-block text-sm text-primary underline">Skip for now</Link>
            </div>
          )}
        </div>
      </main>
...
function Field({ label, v, on, required, textarea }: { label: string; v: string; on: (v: string) => void; required?: boolean; textarea?: boolean }) {
  return (
    <label className="block text-sm">
      ...
      {textarea
        ? <textarea value={v} onChange={(e) => on(e.target.value)} rows={4} className="w-full rounded-lg border border-input bg-white px-3 py-2.5 outline-none focus:ring-2 focus:ring-primary" />
        : <input required={required} value={v} onChange={(e) => on(e.target.value)} className="w-full rounded-lg border border-input bg-white px-3 py-2.5 outline-none focus:ring-2 focus:ring-primary" />}
    </label>
  );
}
function NumField({ label, v, on, required }: { label: string; v: number; on: (v: number) => void; required?: boolean }) {
  return (
    <label className="block text-sm">
      <div className="text-xs font-semibold text-muted-foreground mb-1">{label}</div>
      <input type="number" min={0} required={required} value={v} onChange={(e) => on(Number(e.target.value))}
...
function Sel<T extends string | number | "">({ label, v, on, opts }: { label: string; v: T; on: (v: T) => void; opts: { v: T; l: string }[] }) {
  return (
    <label className="block text-sm">
      <div className="text-xs font-semibold text-muted-foreground mb-1">{label}</div>
      <select value={v as string | number} onChange={(e) => {
        const raw = e.target.value; const cast = (typeof opts[0]?.v === "number" && raw !== "") ? Number(raw) : raw;
        on(cast as T);
      }} className="w-full rounded-lg border border-input bg-white px-3 py-2.5 outline-none focus:ring-2 focus:ring-primary">
        {opts.map((o) => <option key={String(o.v)} value={o.v as string | number}>{o.l}</option>)}
      </select>
    </label>
  );
}
